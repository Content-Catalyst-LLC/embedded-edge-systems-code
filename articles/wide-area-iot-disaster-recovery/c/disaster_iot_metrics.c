...
double delivery_probability(double single_attempt_success, int retries) {
    return 1.0 - pow(1.0 - single_attempt_success, retries);
}

int main(void) {
    double energy = daily_energy_wh(24, 0.001, 0.0005, 0.003, 0.001, 0.02, 2);
    double life = battery_life_days(20.0, energy);
    double delivery = delivery_probability(0.80, 2);

    printf("Daily energy use: %.4f Wh/day\n", energy);
    printf("Battery life: %.2f days\n", life);
